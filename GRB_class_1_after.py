...
def log_likelihood_after(theta, data, model):
    """Log likelihood for model without bins.
    
    Args:
        theta (array): parameters of the model
        data (array): the data points 
        model (function): model to use
...
def log_posterior(theta, data, model, bounds):
    ...
    posterior = log_prior(theta, bounds) + log_likelihood_after(theta, data, model)
    return posterior
...
def metropolis_hastings(theta0, postpdf, data, model, bounds, rng, blind, n = 10000):
    ...
    d = theta0.shape[0]
    logP0 = postpdf(theta0, data, model, bounds)
    ...
    for i in range(n):
        ...
        logP_try = postpdf(theta_try, data, model, bounds)
